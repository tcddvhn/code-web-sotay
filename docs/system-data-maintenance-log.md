...
- Chay 
pm.cmd run build: pass.
- 2026-04-07: S?a l?i tr?c ti?p c�c chu?i ti?ng Vi?t trong C:\CODE_APPWEB\src\components\ImportFiles.tsx thay v� v� b?ng text replacement runtime. Gi? nguy�n logic ghi d� c?a admin v� lu?ng n?p file hi?n t?i. �� ch?y 
pm.cmd run lint v� 
...
- Updated src/components/ReportView.tsx to use a left-side tree: project -> �?ng b? Th�nh ph? -> scoped unit list.
...
- 2026-04-08: Added report-sidebar quick collapse behavior. Double-clicking the main 'B�o c�o' menu item now collapses all expanded report projects, and the left navigation column itself is now scrollable so lower menu items such as 'C�i d?t' remain reachable while report navigation is open. Validation: npm.cmd run lint, npm.cmd run build.
...
2026-04-10: Refined Trich bao cao layout per latest UI request. Project selection and saved blueprints were consolidated into a single horizontal top row matching the extraction-config style. Configuration spacing was tightened, section labels such as 'T�n bi?u tr�ch' and 'C?t tr�ch xu?t' are now underlined, and criterion selectors keep inline search for long vertical/horizontal lists. Preview/export continue to hide the unit-code column. Validation: npm.cmd run lint, npm.cmd run build.

2026-04-10: Further compacted Trich bao cao configuration. Narrowed desktop column proportions in ExtractReportView so the extraction field grid consumes less horizontal space, and added inline search for 'Bi?u m?u ngu?n' in each extraction column, mirroring the criterion-search behavior for long template lists. Validation: npm.cmd run lint, npm.cmd run build.
